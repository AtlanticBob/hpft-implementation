/*
 * hpft TCP-punt (D1c): switch mode + fdb_def_rule_en=1. Root pipe matches IPv4+TCP
 * -> RSS to Arm SW queue 0 (punt to CPU). No fwd_miss => unmatched (RoCE/non-TCP/
 * other VFs) falls through to the kernel FDB (OVS). D1c: re-inject the punted TCP
 * back to the wire via a CPU doca_eth_txq (transparent passthrough, pacing-ready).
 */
#include <unistd.h>
#include <stdlib.h>
#include <string.h>
			   ...
			   struct flow_switch_ctx *ctx)
{
	...
	if (doca_flow_resource_query_entry(punt_entry, &query_stats) == DOCA_SUCCESS)
		DOCA_LOG_INFO("PUNT entry: %lu pkts %lu bytes; reinject rx=%lu tx=%lu fail=%lu",
