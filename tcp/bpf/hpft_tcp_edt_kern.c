...
#define HPFT_SMALL_PKT 256   /* opt2: <=256B latency-sensitive pkts bypass EDT delay */
...
int hpft_tcp_edt(struct __sk_buff *skb)
{
    ...
    /* opt2: small (latency-sensitive) packets bypass the EDT delay - fq sends
     * them immediately - while their bytes were already added to the pair debt
     * above, so the cap stays accurate. Bulk packets are paced normally. */
    if (skb->len > HPFT_SMALL_PKT && send_ns > now)
        skb->tstamp = send_ns;
    return TC_ACT_OK;
}
