...
def update_one_pair(
    *,
    ...
    paths = map_paths(pin_dir)
    missing = [str(paths[name]) for name in ("hpft_pair_cfg",) if not paths[name].exists()]
    if missing:
        raise TcpShaperError("pinned BPF maps are missing: " + ", ".join(missing))

    apply_results: list[ApplyResult] = []
    for update in (cfg_update,):
        before = time.monotonic_ns()
        argv = [
            bpftool_bin(),
            "map",
        ...
        apply_results.append(run_command(argv))
        after = time.monotonic_ns()
        if update.map_name == "hpft_pair_cfg":
            result["update_latency_ns"] = after - before
            result["update_done_monotonic_ns"] = after

    result["applied"] = True
    result["apply_results"] = [item.as_dict() for item in apply_results]
    return result
