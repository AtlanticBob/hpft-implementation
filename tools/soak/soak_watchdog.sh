...
LOG=/home/zhaoxiang/hyperfront/hpft-implementation/results/soak_20260710/watchdog.log
TS=$(date -u +%FT%TZ)
...
EXP=$(ssh -o BatchMode=yes -o ConnectTimeout=8 sgpu02 \
  'systemctl is-active hpft-rate-exporter' 2>&1)
echo "$TS now=$(date +%s) rx=[$RX] tx=[$TX] shim=$SHIM exporter=$EXP" >> $LOG
