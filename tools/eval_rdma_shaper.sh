...
set_vf0_cap() {  # $1 = Gbit; rewrite caps file keeping other pairs
    local u; u=$(units "$1")
    ssh -o BatchMode=yes hpft-dpu2 "awk -v u=$u '{if(\$1==\"$DSTIP\")print \$1,u; else print}' $CAPS > /tmp/c.$$ && mv /tmp/c.$$ $CAPS"
    sleep 1.2   # let rx_agent -> tx_agent -> mailbox -> water-level settle
}
bw_srv() { ssh -o BatchMode=yes sgpu02 "pkill -9 -x ib_write_bw 2>/dev/null; sleep 0.3; setsid nohup $PT/ib_write_bw -F -d $RDEV -p $1 -s 65536 -q ${2:-1} --report_gbits -D ${3:-8} >/tmp/rbw_s.log 2>&1 </dev/null & sleep 1.3"; }
lat_srv() { ssh -o BatchMode=yes sgpu02 "pkill -9 -x ib_write_lat 2>/dev/null; sleep 0.3; setsid nohup $PT/ib_write_lat -F -d $RDEV -p $1 -s 2 -n ${2:-3000} >/tmp/rlat_s.log 2>&1 </dev/null & sleep 1.2"; }
...
ssh -o BatchMode=yes sgpu02 'pkill -9 -x ib_write_bw 2>/dev/null; pkill -9 -x ib_write_lat 2>/dev/null'
