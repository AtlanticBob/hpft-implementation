...
"""fig/<tag>_timeline.png: what the run looked like, on the experiment clock.
  panel 1  attributed rate of every flow-set (receiver agent, 20 ms), RDMA red /
...
for r in [r for r in rows if r["cls"] in col]:
    f = r["fsid"]
    ax[0].plot(T, [x["r"].get(f, 0) / 1e9 for _, x in rx], color=col[r["cls"]], lw=0.7,
               ls="-" if r["host"] == hosts[0] else "--", alpha=0.8)
    ax[2].plot(T, [x.get("d", {}).get(f, 0) for _, x in rx], color=col[r["cls"]], lw=0.6, alpha=0.6)
...
ax[0].set_ylabel("attributed rate per flow-set (Gb/s)"); ax[0].legend(ncol=5, fontsize=8); ax[0].grid(alpha=0.3)
...
ax[2].set_ylabel("virtual queue (ms)"); ax[2].set_xlabel("experiment time (s)"); ax[2].grid(alpha=0.3)
