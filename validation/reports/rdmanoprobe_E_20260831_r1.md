...
| 计时 | T0 = 1788171113，预热 5 s，实验时钟 = T0 + 预热 |
...
|---|---|---|---|---|---|
| 0-30 | sgpu01/vf0>sgpu02/vf0|rdma | 25.00 | 24.74 | 1.24 | -1.0% |
| 0-30 | sgpu03/vf0>sgpu02/vf0|rdma | 25.00 | 24.75 | 1.30 | -1.0% |

## 应用自报 goodput（整条流的均值）

| 行 | 流集合 | 起–止 (s) | goodput (G) |
|---|---|---|---|
| 0 | sgpu01/vf0>sgpu02/vf0|rdma | 0–30 | 23.13 |
| 1 | sgpu03/vf0>sgpu02/vf0|rdma | 0–30 | 23.13 |
...
|---|---|---|
| 1 稳态贴合 | 通过 | ok |
| 2 队列消得掉 | 不通过 | 0-30s mean 1.46 ms; sgpu03/vf0>sgpu02/vf0|rdma went 1.0 s without clearing |
| 3 收敛 ≤ 1 s | 通过 | ok |
| 4 不塌 | 通过 | ok |
| 5 平台健康 | 通过 | ok |
...
CNP/ECN 计数增量：sgpu01.mlx5_3.rp_cnp_handled +0，sgpu01.mlx5_3.rp_cnp_ignored +0，sgpu03.mlx5_3.rp_cnp_handled +0，sgpu03.mlx5_3.rp_cnp_ignored +0，sgpu02.mlx5_3.np_cnp_sent +0，sgpu02.mlx5_3.np_ecn_marked_roce_packets +0。TCP 执行面跑后信任度最大值 0.000（196 个流对）。交换机 swp37s0 队列计数快照在 `results/rdmanoprobe_E_20260831_r1/switch_pre.txt` 与 `switch_post.txt`。

丢包与信任（设计 v4 §6.3 第三步）：RDMA 执行面 2 个流集合共收到 0 次 NACK，丢包快速通道放行信任上升 0 个周期，信任度峰值 0.0000；TCP 执行面 8 个流集合中 8 个出现过重传，快速通道放行 164 个周期，信任度峰值 0.0003。快速通道只在队列为空且拥塞控制的额度低于围栏时才放行，所以有重传不等于会放行。

## 图与数据

`fig/rdmanoprobe_timeline.png`（每次运行覆盖）：上图每个流集合的归因速率与应得（黑点线），中图接收端网卡按 VM 和类的 wire 速率堆叠，下图虚拟队列。数据：`data/rdmanoprobe_E_20260831_r1_vmclass.csv`（100 ms wire）、`data/rdmanoprobe_E_20260831_r1_flowsets.csv`、`data/rdmanoprobe_E_20260831_r1_goodput.csv`、`data/rdmanoprobe_E_20260831_r1_events.csv`、`data/rdmanoprobe_E_20260831_r1_verdict.csv`。原始数据 `results/rdmanoprobe_E_20260831_r1/`。
