...
| V9 | 置信度的退出 | 隐形瓶颈消失后，交出去的方向盘多快拿回来；拿不回来（关掉过期项的对照臂）时，下一次管内拥塞里这个租户会付什么代价 |
...
### V8 置信度：账本管不到的瓶颈（90 s，两条臂）

置信度是整套设计里唯一一处把方向盘交给租户 CC 的地方，它上涨要同时满足三条（design v4 §6.3，执行面每 1 ms 判一次）：虚拟队列为零、租户 CC 要得比围栏少（按兑现容差判定，$c<(1-\delta)R$，$\delta=15\%$）、最近 100 ms 内出现过丢包（RDMA 数 NAK，TCP 数 `total_retrans` 的增量并排除紧跟自己降速的那些）。满足则每周期涨 $(1-T)/\tau_r$，$\tau_r = 1$ s；三条不同时成立的每个周期按 $T\leftarrow T-T\,\max(q/D,\ \Delta t/\tau_d)$ 下降，$\tau_d = 5$ s（design v4 §6.2，2026-09-02 起：置信度要靠证据反复出现才保得住，证据一停就自动退回）。

**前七个场景一次都测不到它。** 卡在第二条：打流器全是满发的，CC 读数一直顶在 60 G 线速，而围栏只有 2 到 25 G。V4 那条限速 10 G 的 QP 最像自律租户，但 perftest 的限速是软件节流，CC 寄存器照样读线速，这份自律对执行面不可见。V7 看起来最像，但那 40 G UDP 压的是接收端口本身，而账本恰恰建模了它（根容量按物理端口减去管不着的流量重算），所以 V7 的正确答案就是置信度保持为零——实测也确实如此。
...
**预期。** 0–30 s：vf0 只有 RDMA 一类，吃满 VM 上限 50 G（线上口径；应用口径 46.6 G），vf1 两类各 25 G；端口总载荷 100 G，200 G 的口子上没有任何物理拥塞，NAK 只应在建链那一下出现一次（实测健康路径上 `packet_seq_err` 建链爆约 1300，之后二十秒严格为 0）。30–60 s：vf0 的路径只能过 20 G。臂 A 应当看到置信度在一秒量级涨起来、执行面放开下界、CC 落到路径能过的速率上、丢包停止、vf0 稳在 20 G ± 5%；臂 B 应当看到围栏纹丝不动地按 40 G 推、丢包持续、goodput 掉到 20 G 以下（探路里 2.4 倍过驱动时掉到路径容量的 63%，NAK 每秒 8.5 万到 9.3 万）。两条臂里 vf1 都应当稳在 25 + 25。60–90 s：meter 恢复 50 G，vf0 回到 50 G；丢包停止后臂 A 的置信度失去证据、按 $\tau_d$ 自动退回（专门的判据在 V9，那里的前 90 s 就是本场景）。
...
6. **置信度确实动了**：臂 A 里 vf0 的 RDMA 置信度在 30 s 之后升到 0.5 以上，臂 B 里全程为零。
7. **邻居不受牵连**：两条臂里 vf1 的两个流集合全程贴合 25 G ± 5%，置信度全程为零。

**图**：`fig/V8a_timeline.png` / `fig/V8a_trust.png`（臂 A）与 `fig/V8b_timeline.png` / `fig/V8b_trust.png`（臂 B）。两套图并排看：时间线看份额与不塌，置信度图看 $T$ 的轨迹、CC 速率与围栏的相对位置。
...
### V9 置信度的退出：瓶颈消失之后（150 s，两条臂）

V8 证明置信度该涨的时候会涨；这个场景检验它该消的时候消不消得掉，以及消不掉的代价。规则本身（design v4 §6.2）：上升条件不成立的每个周期按 $T\leftarrow T-T\,\max(q/D,\ \Delta t/\tau_d)$ 下降，$\tau_d=5$ s，也就是丢包一停、置信度就按 5 秒的时间常数自动退回零。过期这一项必须存在，因为置信度自己没有别的下来的路：瓶颈消失时网络不给任何新信号（唯一变化是丢包停了），而置信度高的流因为上界恒为围栏、永远不会多拿，队列那条下降路也永远触发不了。实测（V8a_sr_20260902_rep1，没有过期项的旧规则）瓶颈撤掉后置信度只靠围栏向上探测时偶尔的过冲记进账本，衰减时间常数约 278 s，从 0.98 消到 0.05 要 13.8 分钟，期间这条流的下界保护整段是关着的。

**时间线四段。** 0–30 s 基线；30–60 s 与 V8 相同的隐形瓶颈（vf0 的 meter 压到 20 G），把 sgpu01/vf0 的置信度喂到 0.9 以上；60–90 s 瓶颈撤掉，看置信度怎么退；90–150 s sgpu03、sgpu04 各 4 QP 加入 vf0——新来者按起步值放行（design v4 §5.4：$R_0=hC=16$ G，未知 QP 每个 4 G，两台合计约 32 G 撞进已被占满的 50 G meter），在管内瓶颈上制造真实丢包，sgpu01 的 DCQCN 被 slow restart 切到围栏之下。这时下界保护在不在，就是两臂的差别：不在（过期项关的臂），线上跟着被切低的 CC 走，份额被出借、能不能回来全看 CC 自己的恢复；在（过期项开的臂），执行面把它顶回围栏。

**两条臂。** 臂 A（过期项开）是默认，`g_trust_decay = 13`（每周期 $T/5\,\mathrm{s}$）。臂 B（过期项关）用 `HPFT_RDMA_TRUST_DECAY=0` 起跑，runner 经邮箱 `0xcce 0 10` 把过期步长写成零，只留虚拟队列一条下降通路——这就是 2026-09-02 之前的规则，作为反面对照。两条臂的抬升都开着（步长 66）。与 V8 的臂 B 一样，这个开关只作用于 RDMA 执行面；TCP 执行面的置信度参数是编译期常量，vf1 上的 TCP 只作阴性对照。
...
**预期。** 0–60 s 两臂都应复刻 V8 臂 A：基线 50 + 25 + 25，瓶颈期置信度涨到 0.9 以上、CC 落到 20 G 上、丢包大幅减少。60–90 s 分道：臂 A 的置信度在丢包停止后（约 63 s，meter 恢复后 CC 还要几秒才爬回围栏之上、丢包才真正停）按 $\tau_d$ 退回，3$\tau_d$（15 s）内降到 0.1 以下；臂 B 停在 0.9 附近不动（278 s 的时间常数在 30 s 的窗里只消得掉一成）。90 s 加入后 vf0 的三个 RDMA 流集合各应得约 1/3（应用口径约 15.5 G），vf1 不变：加入瞬态里两臂的 sgpu01/vf0 都会被丢包切低（这段丢包同时满足上升三条件，臂 A 的置信度也会临时涨回去——这不是缺陷，丢包在持续就该让位）；瞬态过后丢包停止，臂 A 的置信度重新退回、下界回到围栏，sgpu01/vf0 被顶回应得；臂 B 的置信度留在高位，sgpu01/vf0 回不回得来、多快回来完全取决于 DCQCN 自己的恢复——如果 DCQCN 恢复得快，两臂的带宽轨迹可能接近，此时过期项的价值只体现在置信度轨迹本身（保护在位的时间），报告如实写。

**判据。** 通用五条照用（应得由政策与 meter 取小；30/60/90 三个沿上的判据 3 与 V8 同理只报数字、不作为通过依据）。另加三条本场景专有的：

6. **退回**：臂 A 里 sgpu01/vf0 的置信度在 80 s 前降到 0.1 以下（退回从丢包停止算起，而丢包比 meter 恢复晚停约 3 s——CC 要先爬出瓶颈——再加 3$\tau_d$ = 15 s，余量到 80 s）。臂 B 只作对照报告、不设通过线：没有过期项时，置信度的清除全靠 meter 释放瞬间恰好出现的一次队列脉冲，是随机的——实测两种行为都出现过（一次被单个脉冲从 0.985 打到 0.047 后再不动，一次 29 秒只消 0.1、拟合时间常数约 278 s），这个随机性本身就是要记录的对照结论。
7. **保护恢复**：90–150 s 对 sgpu01/vf0 单独套判据 4（事件后 2 s 瞬态豁免，不得低于应得 50% 持续 1 s 以上），臂 A 必须过；臂 B 的数字照报作对照。
8. **邻居不受牵连**：两臂里 vf1 的两个流集合全程贴合 25 G ± 5%，置信度全程为零。

**图**：`fig/V9a_timeline.png` / `fig/V9a_trust.png`（臂 A）与 `fig/V9b_timeline.png` / `fig/V9b_trust.png`（臂 B）。

**跑法与复原。** `bash validation/run/run.sh V9_trust_exit <tag>` 跑臂 A，`HPFT_RDMA_TRUST_DECAY=0 bash validation/run/run.sh V9_trust_exit <tag>` 跑臂 B。runner 把实际写进执行面的两个置信度参数记在 `trust_arm.txt`，meter 的窗口记在 `hidden_meter.txt`，EXIT 陷阱恢复 meter 与两个参数的默认值。
