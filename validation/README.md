...
置信度是整套设计里唯一一处把方向盘交给租户 CC 的地方，它上涨要同时满足三条（design v4 §6.3，执行面每 1 ms 判一次）：虚拟队列为零、租户 CC 要得比围栏少 2% 以上、最近 100 ms 内出现过丢包（RDMA 数 NAK，TCP 数 `total_retrans` 的增量并排除紧跟自己降速的那些）。满足则每周期涨 $(1-T)/\tau_r$，$\tau_r = 1$ s；三条不同时成立的每个周期按 $T\leftarrow T-T\,\max(q/D,\ \Delta t/\tau_d)$ 下降，$\tau_d = 5$ s（design v4 §6.2，2026-09-02 起：置信度要靠证据反复出现才保得住，证据一停就自动退回）。
