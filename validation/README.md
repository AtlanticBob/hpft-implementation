...
**时间线四段。** 0–30 s 基线；30–60 s 与 V8 相同的隐形瓶颈（vf0 的 meter 压到 20 G），把 sgpu01/vf0 的置信度喂到 0.9 以上；60–90 s 瓶颈撤掉，看置信度怎么退；90–150 s sgpu03、sgpu04 各 4 QP 加入 vf0——新来者按未知流上限起步（每 QP 5 G，合计约 40 G 撞进已被占满的 50 G meter），在管内瓶颈上制造真实丢包，sgpu01 的 DCQCN 被 slow restart 切到围栏之下。这时下界保护在不在，就是两臂的差别：不在（过期项关的臂），线上跟着被切低的 CC 走，份额被出借、能不能回来全看 CC 自己的恢复；在（过期项开的臂），执行面把它顶回围栏。
