...
| V8 | 置信度 | 存在一个接收端账本管不到的瓶颈并且在丢包时，执行面会不会把方向盘交给租户 CC；不交会不会拥塞崩溃；旁边没有隐形瓶颈的租户会不会被牵连 |
| V9 | 置信度的退出 | 隐形瓶颈消失后，交出去的方向盘多快拿回来；拿不回来（关掉过期项的对照配置）时，下一次管内拥塞里这个租户会付什么代价 |

V1、V2、V4、V7 是每一版设计必跑的四个；V3、V5、V6 在它们过了之后跑。V8 与 V9 是仅有的两个检验置信度规则的场景——另外七个都测不到它，理由写在 V8 那一节里；V8 管授予（涨起来该不该、不涨会怎样），V9 管退出（消得掉吗、消不掉会怎样）——所以改动设计 v4 第 6 节的任何一条都要重跑这两个。

## 二、共同环境（除非某个场景明确覆盖）

三台发送端 sgpu01/03/04 经 sn5600 打接收端 sgpu02，四台的 p1 都是 200 G，接收端口 `swp37s0` 是 3:1 超订的瓶颈。**2026-09-04 起 sn5600 用一根回环线分成两台逻辑交换机**：交换机 A（VLAN 101）接 sgpu01（`swp37s1`）和 sgpu03（`swp3s1`），交换机 B（VLAN 102）接 sgpu02（`swp37s0`）和 sgpu04（`swp4s1`），两台之间只有 `swp21`↔`swp25` 这一根 OSFP 线（现在 800 G，不是瓶颈；没有 ECN、没有 PFC）。sgpu01、sgpu03 打 sgpu02 的流量都过这根线，sgpu04 的不过。它是接收端账本管不到的核心链路（design v4 §8.1）；端口表、配置脚本与备份在 `tools/lab-infra/switch/`。数据面是常驻 VxLAN overlay，**2026-09-04 起是静态全网状**：每台 DPU 到其它三台各一条隧道，转发规则由注册表生成（本地 VF 出去的包按目的 IP 进对应隧道，隧道进来的包按目的 IP 打 meter 后送到本地 VF，隧道进来的其它一切丢弃，所以没有环路、也不依赖 MAC 学习），`tos=inherit`；底层是 DPU 的 p1（172.16.1.x，MTU 9000）。此前是以接收端为中心的星型，任何两台发送端之间的流量都要经 sgpu02 中转，因此只能有一个接收端；网状之后任何一台都能收发。租户 = sgpu02 的一个 VF；流集合 = (源 VM, 目的 VM, 类)。
...
sgpu01 的 4 个 VM 两类共 8 个流集合常驻；sgpu03 的 4 个 VM 在 10 s 加入 RDMA 流集合、20 s 退出，在 30 s 加入 TCP 流集合、40 s 退出，每个加入者与一个在位者共用同一个目的 VM 的同一个类。**一次事件只动一类流**（2026-09-02 起）：真实系统里流集合和租户的加入退出一次只改变一类流，两类同时变的事件不存在；同时变还会让两个执行面的起步瞬态叠在一起，量出来的是叠加而不是控制环。每个目的 VM 的 46 G 两类各 23 G；RDMA 那一类在 10–20 s 由 2 个流集合分（各 11.5 G），TCP 那一类在 30–40 s 同样；其余时段各 23 G。
...
**预期。** 每类各自：在位者 23 → 11.5 → 23 G，加入者从零到 11.5 G，另一类全程不动；四个阶跃的收敛时间都 ≤ 1 s；加入瞬间合计不超过 110% C′；整场没有塌陷；20–30 s、40–50 s 与 0–10 s 的数字一致。**图**：每流集合速率时序 + 虚拟队列 + 合计，事件竖线与应得参考线。
...
**预期。** 每个在位租户 46 → 23 → 46 G（只在 20 s 与 80 s 动），每个新租户 0 → 23 → 23 → 23 → 0 G（20 s 起 RDMA 独占，40–60 s 两类各 11.5 G），四个事件的收敛都 ≤ 1 s，无塌陷。**图**：每租户 wire 时序（堆叠面积）+ 收敛时间表。
...
打流与 V2 完全相同（RDMA 10 s 加入、20 s 退出；TCP 30 s 加入、40 s 退出），只把 RDMA 的 CC 换成执行面里的 Swift 项（`0xccd 3`，阈值按 `results/swift_20260827/` 的校准），TCP 换成 BBR（`iperf3 -C bbr`）。
...
**预期。** 与 V2 相同：每类各自 23 → 11.5 → 23 G，收敛 ≤ 1 s，无塌陷。差异超过 5% 就说明设计里有依赖具体 CC 的地方。

### V7 与租户 CC 共处：接收端口被 HyperFront 看不见的流量压住（90 s）

打流与 V1 相同（24 个流集合把根用满），另加一股 HyperFront 既不调度也不整形的背景 UDP：30–60 s 从 sgpu04/vf7 往 sgpu02/vf7 打 40 G（接收端归为 ip_other，不进分配；发送端只对 TCP 和 RDMA 整形）。这 30 秒里端口上有 184 + 40 > 200 G 的需求，交换机队列真的积起来：RoCE 包被打 ECN、DCQCN 减速，TCP 丢包减窗。HyperFront 仍按 184 G 发许可速率，只有租户 CC 能把流量压进 160 G，执行面应当把方向盘交给 CC；60 s 背景停掉，许可速率应当重新接管。
...
**预期。** 0–30 s 与 V1 相同。30–60 s：接收端把根容量缩到 (200 − 41) × 0.92 = 146 G（它计量得到 UDP，只是不调度），四个 VM 各 36.6 G，两类各 18.3 G，24 个流集合各 6.09 G ± 5%，Jain ≥ 0.99；端口总载荷（租户 + UDP）≈ 187 G，交换机队列不持续积压，RDMA 不被 CC 压死；执行面的置信度不上升（CC 只是短暂减速、随即涨回，按趋势规则算"追赶"）。60 s 背景停掉，根回到 184 G，1 s 内回到 V1 的份额。**图**：每 VM 每类 wire 时序（vf7 的 eth 桶就是背景 UDP）+ 执行面置信度时序（TCP 每 100 ms 读 BPF，RDMA 每秒查设备）。**判据**：与其他场景相同，应得值按物理根 146 G 计算；另加"没有流集合低于 3 G"。**注意**：设备端探针（`rp_probe.py`）每秒对每个流对发一次邮箱查询，每次占用邮箱 13–22 ms，会让 RDMA 执行面的预算推送抖动（无背景流时队列出现 5–15 ms 的毛刺），所以这个场景的 0–30 s 和 60–90 s 不能拿来和 V1 比稳态。

### V8 置信度：账本管不到的瓶颈（90 s，两个配置）

置信度是整套设计里唯一一处把方向盘交给租户 CC 的地方，它上涨要同时满足三条（design v4 §6.3，执行面每 1 ms 判一次）：虚拟队列为零、租户 CC 要得比许可速率少（按兑现容差判定，$c<(1-\delta)R$，$\delta=15\%$）、最近 100 ms 内出现过丢包（RDMA 数 NAK，TCP 数 `total_retrans` 的增量并排除紧跟自己降速的那些）。满足则每周期涨 $(1-T)/\tau_r$，$\tau_r = 1$ s；三条不同时成立的每个周期按 $T\leftarrow T-T\,\max(q/D,\ \Delta t/\tau_d)$ 下降，$\tau_d = 5$ s（design v4 §6.2，2026-09-02 起：置信度要靠证据反复出现才保得住，证据一停就自动退回）。

**前七个场景一次都测不到它。** 卡在第二条：打流器全是满发的，CC 读数一直顶在 60 G 线速，而许可速率只有 2 到 25 G。V4 那条限速 10 G 的 QP 最像自律租户，但 perftest 的限速是软件节流，CC 寄存器照样读线速，这份自律对执行面不可见。V7 看起来最像，但那 40 G UDP 压的是接收端口本身，而账本恰恰建模了它（根容量按物理端口减去管不着的流量重算），所以 V7 的正确答案就是置信度保持为零——实测也确实如此。

**怎么造一个账本管不到的瓶颈。** 现有星型拓扑里会拥塞的链路只有接收端口一条，而账本建模的正是它，自然的隐形瓶颈不存在，只能人工造：把接收端 sgpu02/vf0 的 OVS drop meter 在 30–60 s 压到 20 G，注册表不动、仍写 50 G。接收端的 vport 计数器读的是丢包之后的量（实测：meter 8 G 时读 8.10 G，50 G 时读 19.36 G），所以账本只看得见活下来的那部分，会把这个流集合当成出借方：队列保持为零、入场额保持 50 G，而许可速率的探测天花板是自身用量的两倍，于是许可速率停在约 40 G——执行面往一个只能过 20 G 的口子里推 40 G，多出来的被丢掉，租户的 CC 收到 NAK。这正是 §6.3 说的那种情形。

同一次运行里 sgpu02/vf1 是阴性对照：它没有隐形瓶颈，两类各 25 G，全程不该有任何变化，置信度不该离开零。

**先决条件：CC 项必须对丢包有反应。** 置信度的第二条闸门是「租户 CC 要得比许可速率少」，而隐形瓶颈只丢包、不打标记。两个 CNP 驱动的项（AIMD `0xccd 0`、DCQCN RP 状态机 `0xccd 2`）原本对 NACK 不做任何反应，`cc_rate` 会一直停在线速、闸门永远打不开；2026-09-02 起它们都带 slow restart（丢包时乘性压低 `Rc`、恢复阶梯清零，对应固件 ROCE_ACCL 的 `roce_slow_restart_en`），闸门因此可以成立。切幅与节流由 `0xcce <值> 8`、`0xcce <值> 9` 调，切幅设 65536 就是关掉——那是这个场景的第三个配置，用来说明 CC 不退让时置信度无从谈起。ZTR（`0xccd 1`）的 NACK 分支挂着 70 µs 的时延门槛，而丢包 meter 不排队，预期不会降。

**TCP 版。** 同一个场景把 vf0 上的 RDMA 换成 4 条 TCP（`scenarios/V8_hidden_bottleneck_tcp.flows`，meter 20 G；`V8_hidden_bottleneck_tcp5.flows` 是 5 G 的一档），其余不变。TCP 执行面跑的是与 RDMA 执行面同一套置信度规则（三个上升条件、$\tau_r$、$\tau_d$、兑现容差门槛、空闲重置，design v4 §6.2–6.4），TCP 的额度 $c$ 是名下各连接拥塞窗口之和除以最小往返时延，丢包证据是协议栈重传计数的增量并剔除整形器自己丢包之后 300 ms 内的那些。这个场景检验的是 design v4 §8.1 写明的边界：数据中心里最小往返时延只有几十微秒，许可速率折成窗口只有几十个报文段，TCP 在只丢包、不排队的 meter 上缩窗口，但缩不到那么小，额度读出来仍在许可速率之上，第二个上升条件不成立，置信度全程为零，执行面按许可速率推、多出的部分在 meter 上一直丢。判据照 RDMA 版套，另加：**6′ 置信度不上升**（vf0 的 TCP 置信度全程低于 0.05，说明门槛按设计工作，没有把这种情形误判成退让）；**7 邻居不受牵连** 同 RDMA 版。两个环境事实随这个场景记录：改 meter 对已卸载的 TCP 流要 3–4 s 才生效（RDMA 一个 100 ms 桶内生效），所以 30 s 与 60 s 两个沿上 TCP 的收敛数字里有 3 s 是 OVS 的重验证，不是 CC；vf0 跑满 TCP 时 vf1 上的 RDMA 比 25 G 低约 7%（交换机对 RoCE 的 ECN 标记），不是置信度的事。图名前缀 `V8t` / `V8t5`。

**两个配置。** 配置 A（置信度开）是默认，`g_trust_step = 66`（每周期 $(1-T)/1\,\mathrm{s}$）。配置 B（置信度冻结）用 `HPFT_RDMA_TRUST_STEP=0` 起跑，runner 通过邮箱 `0xcce 0 7` 把 RDMA 执行面的置信度锁死在零，线上速率于是恒等于许可速率。**配置 B 只冻结 RDMA 一侧**：TCP 执行面的步长是编译期常量 `HPFT_TRUST_STEP`，这一版没做开关，所以 vf1 上的 TCP 不参与对照配置，只作阴性对照的一部分。
...
**预期。** 0–30 s：vf0 只有 RDMA 一类，吃满 VM 上限 50 G（线上口径；应用口径 46.6 G），vf1 两类各 25 G；端口总载荷 100 G，200 G 的口子上没有任何物理拥塞，NAK 只应在建链那一下出现一次（实测健康路径上 `packet_seq_err` 建链爆约 1300，之后二十秒严格为 0）。30–60 s：vf0 的路径只能过 20 G。配置 A 应当看到置信度在一秒量级涨起来、执行面放开下界、CC 落到路径能过的速率上、丢包停止、vf0 稳在 20 G ± 5%；配置 B 应当看到许可速率纹丝不动地按 40 G 推、丢包持续、goodput 掉到 20 G 以下（探路里 2.4 倍过驱动时掉到路径容量的 63%，NAK 每秒 8.5 万到 9.3 万）。两个配置里 vf1 都应当稳在 25 + 25。60–90 s：meter 恢复 50 G，vf0 回到 50 G；丢包停止后配置 A 的置信度失去证据、按 $\tau_d$ 自动退回（专门的判据在 V9，那里的前 90 s 就是本场景）。
...
- 判据 4（不塌）在这里是主判据：30–60 s 里 vf0 的速率不得低于路径容量的一半（10 G）持续 1 s 以上。这就是"没有置信度就会拥塞崩溃"的直接检验。

另加两条本场景专有的：

6. **置信度确实动了**：配置 A 里 vf0 的 RDMA 置信度在 30 s 之后升到 0.5 以上，配置 B 里全程为零。
7. **邻居不受牵连**：两个配置里 vf1 的两个流集合全程贴合 25 G ± 5%，置信度全程为零。

**图**：`fig/V8a_timeline.png` / `fig/V8a_trust.png`（配置 A）与 `fig/V8b_timeline.png` / `fig/V8b_trust.png`（配置 B）。两套图并排看：时间线看份额与不塌，置信度图看 $T$ 的轨迹、CC 速率与许可速率的相对位置。

**跑法与复原。** `HPFT_RDMA_TRUST_STEP=0 bash validation/run/run.sh V8_hidden_bottleneck <tag>` 跑配置 B，不带这个变量跑配置 A。runner 自己在窗口起止时改 meter、跑完恢复 50 G，并把实际用的值记进 `results/<tag>/hidden_meter.txt` 与 `trust_arm.txt`；EXIT 陷阱保证异常中止也会恢复。**不要用 `ovs-ofctl meter-stats` 量丢了多少**：硬件卸载路径不更新丢包带的字节计数，实测限速明显生效而 `byte_count` 恒为 0，丢包只能从 NAK（`rp_*.jsonl`）或速率差看。

### V9 置信度的退出：瓶颈消失之后（150 s，两个配置）

V8 证明置信度该涨的时候会涨；这个场景检验它该消的时候消不消得掉，以及消不掉的代价。规则本身（design v4 §6.2）：上升条件不成立的每个周期按 $T\leftarrow T-T\,\max(q/D,\ \Delta t/\tau_d)$ 下降，$\tau_d=5$ s，也就是丢包一停、置信度就按 5 秒的时间常数自动退回零。过期这一项必须存在，因为置信度自己没有别的下来的路：瓶颈消失时网络不给任何新信号（唯一变化是丢包停了），而置信度高的流因为上界恒为许可速率、永远不会多拿，队列那条下降路也永远触发不了。实测（V8a_sr_20260902_rep1，没有过期项的旧规则）瓶颈撤掉后置信度只靠许可速率向上探测时偶尔的过冲记进账本，衰减时间常数约 278 s，从 0.98 消到 0.05 要 13.8 分钟，期间这条流的下界保护整段是关着的。

**时间线四段。** 0–30 s 基线；30–60 s 与 V8 相同的隐形瓶颈（vf0 的 meter 压到 20 G），把 sgpu01/vf0 的置信度喂到 0.9 以上；60–90 s 瓶颈撤掉，看置信度怎么退；90–150 s sgpu03、sgpu04 各 4 QP 加入 vf0——新来者按起步值放行（design v4 §5.4：$R_0=hC=16$ G，未知 QP 每个 4 G，两台合计约 32 G 撞进已被占满的 50 G meter），在管内瓶颈上制造真实丢包，sgpu01 的 DCQCN 被 slow restart 切到许可速率之下。这时下界保护在不在，就是两个配置的差别：不在（过期项关的配置），线上跟着被切低的 CC 走，份额被出借、能不能回来全看 CC 自己的恢复；在（过期项开的配置），执行面把它顶回许可速率。

**两个配置。** 配置 A（过期项开）是默认，`g_trust_decay = 13`（每周期 $T/5\,\mathrm{s}$）。配置 B（过期项关）用 `HPFT_RDMA_TRUST_DECAY=0` 起跑，runner 经邮箱 `0xcce 0 10` 把过期步长写成零，只留虚拟队列一条下降通路——这就是 2026-09-02 之前的规则，作为反面对照。两个配置的抬升都开着（步长 66）。与 V8 的配置 B 一样，这个开关只作用于 RDMA 执行面；TCP 执行面的置信度参数是编译期常量，vf1 上的 TCP 只作阴性对照。
...
**预期。** 0–60 s 两个配置都应复刻 V8 配置 A：基线 50 + 25 + 25，瓶颈期置信度涨到 0.9 以上、CC 落到 20 G 上、丢包大幅减少。60–90 s 分道：配置 A 的置信度在丢包停止后（约 63 s，meter 恢复后 CC 还要几秒才爬回许可速率之上、丢包才真正停）按 $\tau_d$ 退回，3$\tau_d$（15 s）内降到 0.1 以下；配置 B 停在 0.9 附近不动（278 s 的时间常数在 30 s 的窗里只消得掉一成）。90 s 加入后 vf0 的三个 RDMA 流集合各应得约 1/3（应用口径约 15.5 G），vf1 不变：加入瞬态里两个配置的 sgpu01/vf0 都会被丢包切低（这段丢包同时满足上升三条件，配置 A 的置信度也会临时涨回去——这不是缺陷，丢包在持续就该让位）；瞬态过后丢包停止，配置 A 的置信度重新退回、下界回到许可速率，sgpu01/vf0 被顶回应得；配置 B 的置信度留在高位，sgpu01/vf0 回不回得来、多快回来完全取决于 DCQCN 自己的恢复——如果 DCQCN 恢复得快，两个配置的带宽轨迹可能接近，此时过期项的价值只体现在置信度轨迹本身（保护在位的时间），报告如实写。

**判据。** 通用五条照用（应得由政策与 meter 取小；30/60/90 三个沿上的判据 3 与 V8 同理只报数字、不作为通过依据）。另加三条本场景专有的：

6. **退回**：配置 A 里 sgpu01/vf0 的置信度在 80 s 前降到 0.1 以下（退回从丢包停止算起，而丢包比 meter 恢复晚停约 3 s——CC 要先爬出瓶颈——再加 3$\tau_d$ = 15 s，余量到 80 s）。配置 B 只作对照报告、不设通过线：没有过期项时，置信度的清除全靠 meter 释放瞬间恰好出现的一次队列脉冲，是随机的——实测两种行为都出现过（一次被单个脉冲从 0.985 打到 0.047 后再不动，一次 29 秒只消 0.1、拟合时间常数约 278 s），这个随机性本身就是要记录的对照结论。
7. **保护恢复**：90–150 s 对 sgpu01/vf0 单独套判据 4（事件后 2 s 瞬态豁免，不得低于应得 50% 持续 1 s 以上），配置 A 必须过；配置 B 的数字照报作对照。
8. **邻居不受牵连**：两个配置里 vf1 的两个流集合全程贴合 25 G ± 5%，置信度全程为零。

**图**：`fig/V9a_timeline.png` / `fig/V9a_trust.png`（配置 A）与 `fig/V9b_timeline.png` / `fig/V9b_trust.png`（配置 B）。

**跑法与复原。** `bash validation/run/run.sh V9_trust_exit <tag>` 跑配置 A，`HPFT_RDMA_TRUST_DECAY=0 bash validation/run/run.sh V9_trust_exit <tag>` 跑配置 B。runner 把实际写进执行面的两个置信度参数记在 `trust_arm.txt`，meter 的窗口记在 `hidden_meter.txt`，EXIT 陷阱恢复 meter 与两个参数的默认值。
