...
V1、V2、V4、V7 是每一版设计必跑的四个；V3、V5、V6 在它们过了之后跑。V8 是唯一检验信任度那条规则的场景——另外七个都测不到它，理由写在 V8 那一节里——所以改动设计 v4 第 6 节的任何一条都要重跑它。
...
信任度是整套设计里唯一一处把方向盘交给租户 CC 的地方，它上涨要同时满足三条（design v4 §6.3，执行面每 1 ms 判一次）：虚拟队列为零、租户 CC 要得比围栏少 2% 以上、最近 100 ms 内出现过丢包（RDMA 数 NAK，TCP 数 `total_retrans` 的增量并排除紧跟自己降速的那些）。满足则每周期涨 $(1-T)/\tau_r$，$\tau_r = 1$ s；下降只有一条路，被虚拟队列按 $T\leftarrow T-T\,q/D$ 打下去。
