...
**两个配置。** 配置 A（置信度开）是默认，`g_trust_step = 66`（每周期 $(1-T)/1\,\mathrm{s}$）。配置 B（置信度冻结）用 `HPFT_RDMA_TRUST_STEP=0` 起跑，runner 通过邮箱 `0xcce 0 7` 把 RDMA 执行面的置信度锁死在零，线上速率于是恒等于围栏。**配置 B 只冻结 RDMA 一侧**：TCP 执行面的步长是编译期常量 `HPFT_TRUST_STEP`，这一版没做开关，所以 vf1 上的 TCP 不参与对照配置，只作阴性对照的一部分。
...
**预期。** 0–30 s：vf0 只有 RDMA 一类，吃满 VM 上限 50 G（线上口径；应用口径 46.6 G），vf1 两类各 25 G；端口总载荷 100 G，200 G 的口子上没有任何物理拥塞，NAK 只应在建链那一下出现一次（实测健康路径上 `packet_seq_err` 建链爆约 1300，之后二十秒严格为 0）。30–60 s：vf0 的路径只能过 20 G。配置 A 应当看到置信度在一秒量级涨起来、执行面放开下界、CC 落到路径能过的速率上、丢包停止、vf0 稳在 20 G ± 5%；配置 B 应当看到围栏纹丝不动地按 40 G 推、丢包持续、goodput 掉到 20 G 以下（探路里 2.4 倍过驱动时掉到路径容量的 63%，NAK 每秒 8.5 万到 9.3 万）。两个配置里 vf1 都应当稳在 25 + 25。60–90 s：meter 恢复 50 G，vf0 回到 50 G；丢包停止后配置 A 的置信度失去证据、按 $\tau_d$ 自动退回（专门的判据在 V9，那里的前 90 s 就是本场景）。
...
**图**：`fig/V8a_timeline.png` / `fig/V8a_trust.png`（配置 A）与 `fig/V8b_timeline.png` / `fig/V8b_trust.png`（配置 B）。两套图并排看：时间线看份额与不塌，置信度图看 $T$ 的轨迹、CC 速率与围栏的相对位置。
...
**时间线四段。** 0–30 s 基线；30–60 s 与 V8 相同的隐形瓶颈（vf0 的 meter 压到 20 G），把 sgpu01/vf0 的置信度喂到 0.9 以上；60–90 s 瓶颈撤掉，看置信度怎么退；90–150 s sgpu03、sgpu04 各 4 QP 加入 vf0——新来者按起步值放行（design v4 §5.4：$R_0=hC=16$ G，未知 QP 每个 4 G，两台合计约 32 G 撞进已被占满的 50 G meter），在管内瓶颈上制造真实丢包，sgpu01 的 DCQCN 被 slow restart 切到围栏之下。这时下界保护在不在，就是两个配置的差别：不在（过期项关的配置），线上跟着被切低的 CC 走，份额被出借、能不能回来全看 CC 自己的恢复；在（过期项开的配置），执行面把它顶回围栏。
...
**预期。** 0–60 s 两个配置都应复刻 V8 配置 A：基线 50 + 25 + 25，瓶颈期置信度涨到 0.9 以上、CC 落到 20 G 上、丢包大幅减少。60–90 s 分道：配置 A 的置信度在丢包停止后（约 63 s，meter 恢复后 CC 还要几秒才爬回围栏之上、丢包才真正停）按 $\tau_d$ 退回，3$\tau_d$（15 s）内降到 0.1 以下；配置 B 停在 0.9 附近不动（278 s 的时间常数在 30 s 的窗里只消得掉一成）。90 s 加入后 vf0 的三个 RDMA 流集合各应得约 1/3（应用口径约 15.5 G），vf1 不变：加入瞬态里两个配置的 sgpu01/vf0 都会被丢包切低（这段丢包同时满足上升三条件，配置 A 的置信度也会临时涨回去——这不是缺陷，丢包在持续就该让位）；瞬态过后丢包停止，配置 A 的置信度重新退回、下界回到围栏，sgpu01/vf0 被顶回应得；配置 B 的置信度留在高位，sgpu01/vf0 回不回得来、多快回来完全取决于 DCQCN 自己的恢复——如果 DCQCN 恢复得快，两个配置的带宽轨迹可能接近，此时过期项的价值只体现在置信度轨迹本身（保护在位的时间），报告如实写。
