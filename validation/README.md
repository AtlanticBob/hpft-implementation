...
**执行面律的消融**（令牌池对等额封顶 $\min(c_i, R/N)$、对忽略拥塞控制的等分 $R/N$、对拥塞控制单独运行）不在这里重复，runner 支持这些臂（`HPFT_LAW=1|2`、`HPFT_CC_ONLY=1`），定稿的测量在 `results/perqp_executor_20260907/README.md`（quick 运行器，12 秒一场）与设计文档第 6.6 节。
