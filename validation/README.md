...
| 打流器 | RDMA `~/hyperfront/perftest-26015/ib_write_bw`（带 `--start_at`，四台同一份），`-q 4 -m 1024 --report_gbits -D`；TCP 系统 iperf3，`-P 4 -b 0 -J`，`-B <ip>%dpu1vfN` 强绑定 |
...
| 时长与计时 | 每场 90 s（V1、V5 为 60 s）；t=0 的流先预热 5 s 再开始计时，晚加入的流按计时时刻用 `--start_at`（RDMA）或睡到时刻（TCP）准点加入；图和表只取计时后的部分 |
...
sgpu01 的 4 个 VM 两类共 8 个流集合常驻；sgpu03 的 4 个 VM 两类共 8 个流集合在 30 s 加入、60 s 退出，每个加入者与一个在位者共用同一个目的 VM。每个目的 VM 的 46 G 在 0–30 s 由 2 个流集合分（各 23 G），30–60 s 由 4 个分（各 11.5 G），60–90 s 回到各 23 G。
...
**预期。** 在位者 23 → 11.5 → 23 G，加入者从零到 11.5 G，四个阶跃的收敛时间都 ≤ 1 s；加入瞬间合计不超过 110% C′；整场没有塌陷；60–90 s 与 0–30 s 的数字一致。**图**：每流集合速率时序 + 虚拟队列 + 合计，事件竖线与应得参考线。
