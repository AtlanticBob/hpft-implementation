...
被测对象是现行设计：接收端账本按周期给每个流集合一个许可速率 $R$（设计第 4、5 节），发送端执行面把流集合封顶在 $R$、只读租户拥塞控制的额度不改它（设计第 6 节）：每流集合一个令牌池，每条流的上限是 $r_i=\min\big(c_i,\ c_i(R+P)/S\big)$：$c_i$ 是它自己的拥塞控制此刻的速率（TCP 用窗口代），$P$ 是池里上一个四分之一毫秒攒下的存量，$S$ 是上一毫秒在发的流的 $c_i$ 之和，池负责纠正分母的过时；两类执行面同一条规则。执行面没有参数，不判断拥塞控制可不可信；$R$ 以上归账本，$R$ 以下归租户的拥塞控制。这里的每个场景都在检验这两层合起来的承诺。
...
| V8 | 账本管不到的核心瓶颈 | 两个接收端共用一根它们的账本都看不见的核心链路并把它打满时，令牌池执行面的结果是否与租户拥塞控制单独运行相同：不饿死、总量不低、流集合之间不比拥塞控制单独时更不公平 |

V1、V2、V4 是每一版设计必跑的三个；V3、V5、V6 在它们过了之后跑；V7、V8 是设计边界的检验，答案是"和拥塞控制单独运行一样"而不是"达到份额"。

**执行面律的消融**（按比例的池对等额上限的池、对等额封顶 $\min(c_i, R/N)$、对忽略拥塞控制的等分 $R/N$、对拥塞控制单独运行）不在这里重复，runner 支持这些臂（`HPFT_LAW=3|1|2`、`HPFT_CC_ONLY=1`），定稿的测量在 `results/perqp_executor_20260907/README.md`（quick 运行器，12 秒一场）与设计文档第 6.3 节。
...
| 重复 | 小改动单场跑 1 遍；作为定版依据的场景跑 3 遍，收敛时间报 min/median/max，图取三遍的逐点平均画成一张（`timeline.py tagA,tagB,tagC V2`） |
...
4. 发送端 DPU 的 RDMA 执行面回读（`rp_<host>.jsonl`，`tools/dpu/rp_sample.py`，每流集合每秒一次 `0xded`）：$R$、这个流集合各 QP 已整形速率之和、最近一毫秒取过令牌的 QP 的拥塞控制速率之和与个数。这是执行面对自己的记账，用来直接检验设计 6.3 的前两条性质。每次查询占用邮箱 13–22 ms，是一个小扰动而不是被动的探针，`HPFT_NO_RP_SAMPLE=1` 可以关掉。
...
3. **收敛**：从事件时刻起，到目标流集合的速率进入 ±10% 带并在 1 s 的窗内有 ≥ 75% 的格子留在带内的首个时刻，≤ 1 s 为合格。**带子的中心是这个流集合自己的新稳态速率**（取事件后 3 到 7 s 的均值），不是名义份额——份额对不对是判据 1 的问题，这一条只问多久到位，两件事混在一起会让带子套在一个流集合根本不落座的值上。**格子是 250 ms**：四个 QP 的 RDMA 流集合在 100 ms 上的变异系数中位 6.7% 到 7.2%（最大 19%），而 ±10% 的带要装下 90% 的格子需要它低于 6.1%，所以旧口径对 RDMA 结构性地判不过，无论控制环做得多好；250 ms 把散布降到约 4.4%，带子才留给了它该量的瞬态。TCP 从来不是问题（3.3% 到 3.9%）。
4. **不塌**：除事件后 2 s 的瞬态外，任何流集合的速率不得低于应得的 50% 持续 1 s 以上；合计不得低于 90% C′ 持续 1 s 以上。
5. **平台健康**：所有流正常结束、没有 QP 错误完成；四台 `deploy_check.sh` 跑前通过。
6. **执行面守约**（设计 6.3 性质 1、2，只看 RDMA 执行面的回读，稳态窗口内）：在列表上的 QP 全都在取令牌的样本里，已整形速率之和与 R 之比的均值 ≤ 1.03、95 分位 ≤ 1.10（R 本身是 20 ms 的锯齿、已整形滞后它一个事件，单个 1 s 样本会偏几个百分点）；取令牌的 QP 的拥塞控制速率之和 ≥ R 的样本里，已整形之和 ≥ 0.95 R 的 ≥ 90%。拥塞控制单独臂不适用。回读里已整形之和覆盖列表上的每个 QP，而一毫秒内没取过令牌的 QP 保留着上次给它的速率（当时在取的 QP 更少、上限更高），四个 QP 的 perftest 每隔几秒会有一秒只有两三个 QP 在取令牌，这是设计 6.3 的粒度边界而不是律的问题，所以性质 1 只在全员取令牌的样本上判。回读的已整形速率是执行面按时间加权累计的"速率乘时间"、由主机除以两次查询之间的间隔，不是瞬时采样；自述比线上略低是常态（自述是编程的速率，线上比它多三到七个百分点）。
...
图一律按 100 ms 粒度画（`plot/timeline.py`）：接收端遥测按 20 ms 记录，在几十秒的横轴上画出来的绝大部分是测量粒度而不是交付速率。判据 1、2、4 读原始账本，判据 3 读 250 ms 均值。

## 四、场景定义

每个场景的负载窗口在 20 到 40 秒之间：固定开销（部署核对、拉起代理、起打流器、收数据）要三分多钟，负载只是零头；段长的下限由判定用的稳态窗口（每段跳过前 3 秒、丢掉最后 1 秒）与 7 秒的收敛窗决定，所以有事件的场景每段 8 秒或 13 秒。时刻都以实验时钟计（T0 加 5 秒预热之后）。
...
sgpu01 的 4 个 VM 两类共 8 个流集合常驻；sgpu03 的 4 个 VM 在 8 s 加入 RDMA 流集合、16 s 退出，在 24 s 加入 TCP 流集合、32 s 退出。一次事件只动一类流，所以每个沿上只有一类的份额在变：RDMA 加入时每个 VM 的 RDMA 类 23 G 在两个发送方之间对半分，TCP 类不动；TCP 加入时反过来。五段各 8 秒。
...
只有一个租户 sgpu02/vf0，上限 50 G，两类各应得 25 G。TCP 全程满发；RDMA 的应用需求分三段：0–13 s 只要 10 G（1 个 QP 加 `--rate_limit 10`），13–26 s 满发（4 个 QP），26–39 s 又只要 10 G。三段是三个 perftest 实例，同一个流集合。

| 行 | 源 VM | 目的 VM | 类型 | 数量 | 起 (s) | 止 (s) | 备注 |
|---|---|---|---|---|---|---|---|
| 1 | sgpu01/vf0 (10.1.0.1) | sgpu02/vf0 (10.1.0.2) | TCP iperf3 | 4 流 | 0 | 39 |  |
| 2 | sgpu01/vf0 (10.1.0.1) | sgpu02/vf0 (10.1.0.2) | RDMA WRITE | 1 QP | 0 | 13 | `--rate_limit 10`（应用需求 10 G） |
| 3 | sgpu01/vf0 (10.1.0.1) | sgpu02/vf0 (10.1.0.2) | RDMA WRITE | 4 QP | 13 | 26 |  |
| 4 | sgpu01/vf0 (10.1.0.1) | sgpu02/vf0 (10.1.0.2) | RDMA WRITE | 1 QP | 26 | 39 | `--rate_limit 10`（应用需求 10 G） |

**预期。** 0–13 s：RDMA 10 G（应用口径；线上 10.7 G），接收端给它留 15% 的增长余量（线上 12.3 G），TCP 借到其余 37.7 G。13 s：RDMA 需求涨起来，1 s 内回到 25 G，TCP 退到 25 G。26 s：RDMA 回落到 10 G，TCP 1 s 内又借到 37.7 G。全程账本清得掉、合计 ≈ 50 G。这是"接收端把发得少当成不想发"那类问题的直接检验：如果 13 s 之后 RDMA 只能一小步一小步往上爬，这里会看到。**图**：`fig/V4_timeline.png`、`fig/V4_executor.png`。
...
| 3 | sgpu03/vf0 (10.1.0.3) | sgpu02/vf0 (10.1.0.2) | RDMA WRITE | 4 QP | 0 | 30 |  |
| 4 | sgpu03/vf0 (10.1.0.3) | sgpu02/vf0 (10.1.0.2) | TCP iperf3 | 4 流 | 0 | 30 |  |
| 5 | sgpu04/vf0 (10.1.0.4) | sgpu02/vf0 (10.1.0.2) | RDMA WRITE | 4 QP | 0 | 30 |  |
| 6 | sgpu04/vf0 (10.1.0.4) | sgpu02/vf0 (10.1.0.2) | TCP iperf3 | 4 流 | 0 | 30 |  |
...
打流与 V2 完全相同（RDMA 8 s 加入、16 s 退出；TCP 24 s 加入、32 s 退出），只把 RDMA 的拥塞控制换成执行面里的 Swift（`0xccd 3`，runner 看到 `rdma_cc=swift` 自动选），TCP 换成 BBR（`iperf3 -C bbr`）。
...
**预期。** 0–13 s 与 27–40 s 每个 VM 50 G（线上口径），两个臂相同。13–27 s 六个流集合合计接近核心容量，两个臂的合计之差 ≤ 5%，令牌池臂六个流集合的 Jain 指数不低于拥塞控制单独臂，没有流集合低于 3 G；27 s 瓶颈撤掉后 1 s 内回到 50 G。**判据**：通用六条照用（应得值由 `class=core` 行决定），13 s 与 27 s 两个沿上的判据 1、3 只报数字，两个臂之间的比较是通过条件；另加"核心真的拥塞"：`switch_post.txt` 减 `switch_pre.txt`，`swp21` 的 `ecn-marked-frames` 或 `tx-uc-buffer-discards` 在 13–27 s 有增量。**图**：`fig/V8_timeline.png`、`fig/V8_executor.png`、`fig/V8cc_timeline.png`。
...
bash validation/run/run.sh V8_core V8_<tag>                                 # V8 令牌池臂
...
`run/campaign.sh <日期标签> [日志]` 依次跑 V1（DCQCN、Swift）、V2、V4、V3、V5、V6、V7，每场跑完蒸馏、出图、写报告，然后把 sn5600 切成双交换机（`tools/lab-infra/switch/split_stage1.sh`、`split_cutover.sh`）、跑 V8 的两个臂、切回单交换机（`split_rollback.sh`）。它等 `/tmp/hpft_run.lock` 空出来才开始。整套连跑要三个小时以上，跑之前先说一声。
