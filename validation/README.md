...
被测对象是现行设计：接收端账本按周期给每个流集合一个许可速率 $R$（设计第 4、5 节），发送端执行面把流集合封顶在 $R$、只读租户拥塞控制的额度不改它（设计第 6 节）：RDMA 是每流集合一个令牌池，每个 QP 的上限是 $r_i=\min(c_i,(R+P)/N)$，$P$ 是池里上一个四分之一毫秒攒下的存量、$N$ 是最近一毫秒在发的 QP 数；TCP 是按连接的拥塞窗口比例分 $R$。执行面没有参数，不判断拥塞控制可不可信；$R$ 以上归账本，$R$ 以下归租户的拥塞控制。这里的每个场景都在检验这两层合起来的承诺。
...
**执行面律的消融**（令牌池对等额封顶 $\min(c_i, R/N)$、对忽略拥塞控制的等分 $R/N$、对拥塞控制单独运行）不在这里重复，runner 支持这些臂（`HPFT_LAW=1|2`、`HPFT_CC_ONLY=1`），定稿的测量在 `results/perqp_executor_20260907/README.md`（quick 运行器，12 秒一场）与设计文档第 6.3 节。
