#!/usr/bin/env bash
# Safety check before retiring the slew: 64KB repro x2 with slew OFF +
# device dig floor ON (this combination was never tested). p1 at 100G
# (immaterial for a 6G-cap single pair; noted in summary).
set -u
...
for n in 1 2; do
    out="$DIR/noslew_r$n"; mkdir -p "$out"
    ...
    p=$((20200 + n))
    ssh -f sgpu02 "$PT -d mlx5_6 -p $p --report_gbits -D 240 > /tmp/ns_srv.log 2>&1"
    sleep 1
    ...
    echo "noslew_r$n-done"
done
echo noslew-repro-done
