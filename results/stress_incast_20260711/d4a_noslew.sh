#!/usr/bin/env bash
# A/B: d4a (100G root, 4 flows) with budget slew DISABLED (1.0). Suspect:
...
CMD=""
for i in 0 1 2 3; do
    CMD+="nohup $PT -d mlx5_$((6 + i)) -p $((20100 + i)) --report_gbits -D 120 > /tmp/ns_srv$i.log 2>&1 & "
done
ssh sgpu02 "$CMD true"
sleep 2
( ping -i 0.2 -c 600 10.1.1.2 > "$DIR/d4a_ns_ping.txt" 2>&1 ) &
date +%s.%N > "$DIR/d4a_ns_t0.txt"
for i in 0 1 2 3; do
    $PT -d mlx5_$((6 + i)) -p $((20100 + i)) --report_gbits -D 120 \
        "10.1.$i.2" > "$DIR/d4a_ns_vf$i.log" 2>&1 &
done
wait
scp -q hpft-dpu2:/tmp/hpft_rxagent_e.jsonl "$DIR/d4a_ns_rx.jsonl"
echo d4a-noslew-done
