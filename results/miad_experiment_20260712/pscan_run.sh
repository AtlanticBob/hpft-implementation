#!/usr/bin/env bash
# Param cliff scan for MI-based skeletons: args = law mi_alpha beta tag.
# Single-pair 1:1 @6G competition (75s: RDMA whole, TCP [15,45]). Find where
# mi_alpha (MI aggressiveness) breaks fairness/steady like D7's A>=0.001 for
# AIMD.
set -u
...
cd "$REPO"
python3 - "$LAW" "$MIA" "$BETA" <<'EOF'
import json,sys
law,mia,beta=sys.argv[1],float(sys.argv[2]),float(sys.argv[3])
r=json.load(open("config/lab-registry.json"))
e=r["e_params"]; e["law_skeleton"]=law; e["mi_alpha"]=mia; e["beta"]=beta; e["ad_beta"]=0.01
